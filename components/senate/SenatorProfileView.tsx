...
interface SenatorProfileViewProps {
  ...
  /** Plottable senators of the latest Congress (compass). */
  compassMembers: SenateMember[];
  /** Every senator of the latest Congress (delegation pairing). */
  delegationMembers: SenateMember[];
    ...
    <div className="mx-auto flex w-full max-w-[1180px] flex-col gap-7 px-6 pb-16 pt-11">
      ...
      <header className="max-w-[52rem]">
        <p className="mb-2 font-mono text-[0.72rem] uppercase tracking-[0.12em] text-ink-faint">
          Senator · {ordinal(latestCongress)} Congress
        </p>
      ...
      <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-[minmax(0,1fr)_16rem]">
        <div className="flex flex-col gap-5">
          ...
          <Panel label={`In the ${ordinal(latestCongress)} Senate`}>
            <p className="mb-2 max-w-[42rem] text-[0.78rem] text-ink-faint">
              {inChamber && rank
                ? `${name} sits ${ordinal(rank)} from the left of ${scored.length} scored senators. Ringed below; the rest of the chamber is faded.`
                : `No plottable ${ordinal(latestCongress)}-Congress position for ${name} (too few votes). The chamber is shown for context.`}
            </p>
            {inChamber ? (
              <ProfileCompass members={compassMembers} bioguideId={bioguideId} />
            ) : (
              <ProfileCompass members={compassMembers} bioguideId="" />
            )}
        ...
        <aside className="flex flex-col gap-5">
          ...
          <Panel label="Service">
            <dl className="flex flex-col gap-[0.55rem]">
              <Stat
                label="First Senate Congress"
                value={`${ordinal(firstCongress)} (${congressStartYear(firstCongress)})`}
              />
              <Stat label="Congresses served" value={senateCongressCount} />
